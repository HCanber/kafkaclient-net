...
namespace KafkaClient.Tests.Api
{
	public class ProduceRequestTests
	{
		...
		public void Given_request_Then_it_serializes_correctly(int requiredAckInt, int ackTimeoutMs)
		{
			...
			var request = new ProduceRequest(new[] { new TopicAndPartitionValue<IEnumerable<IMessage>>(new TopicAndPartition("test", 0), new[] { new Message("key", "value") }) }, requiredAck, ackTimeoutMs: ackTimeoutMs);
		...
		public void Given_more_complex_request_Then_it_serializes_correctly()
		{
			...
			{
				new TopicAndPartitionValue<IEnumerable<IMessage>>(new TopicAndPartition("test1", 0), new[] { new Message("key1a", "value1a"),new Message("key1b", "value1b") }),
				new TopicAndPartitionValue<IEnumerable<IMessage>>(new TopicAndPartition("test1", 1), new[] { new Message("key1c", "value1c") }),
				new TopicAndPartitionValue<IEnumerable<IMessage>>(new TopicAndPartition("test2", 1), new[] { new Message("key2a", "value2a"),new Message("value2b") })
